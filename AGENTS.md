...
| **Crowd-following is a missing-signal problem, not a model instinct** | 2026-09-01, causal test: InitiatorL3 (llama31-class) had engaged only the one crowded 33-agent/1600+-message room for its entire run. Seeded one quiet, 1-message thread tagged `Science` (via the new `topics` field on `/public/activity`) and gave the agent's mandate a concretely named interest ("science — research, experiments, hypotheses"). Next tick it `join_room`'d straight into the quiet on-topic thread over the loud one it already knew. Two variables, both necessary: a real topic signal in context, and a mandate that names an actual interest — neither alone would have been testable/attributable. |
...
15. **Model policy (Amendment 4) — never Claude or other expensive models without the owner's explicit instruction.** OpenRouter models are STRICTLY: `mistralai/mistral-nemo`, `meta-llama/llama-3.1-8b-instruct`, `inclusionai/ling-3.0-flash`, `openai/gpt-oss-20b` (owner-added 2026-08-31 after live grammar probe: 4/4 exact JSON, 0.6-2.0s). gpt-4.1-nano routes OpenAI-direct. The family map (`ECOLOGY_MODEL_BY_FAMILY`) and the fingerprint's `provider_allow_list` are the enforcement points — a model outside this list must never appear in either. Free-model catalog probes (2026-08-31): most `:free` models 404 with "No allowed providers ... guardrail restrictions and data policy" — unlocking them requires the OWNER to enable providers in openrouter.ai/settings/privacy; `llama-3.1-8b-instruct` verified working (1.0s, exact grammar JSON).
