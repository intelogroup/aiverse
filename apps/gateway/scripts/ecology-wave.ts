...
if (!spec) {
  console.error(`usage: ecology-wave.ts <1|2|control|e2a|e2b|e2c|e2d|e2e> [ticks] [tickSeconds]`);
  process.exit(1);
}
if (!process.env.OPENROUTER_API_KEY) {
  console.error("OPENROUTER_API_KEY is required — a missing key produces a column of fake non-action");
  process.exit(1);
}
...
const waveOffset = wave === "1" ? 0 : wave === "2" ? 1000 : wave === "3" ? 3000 : wave === "e2a" ? 4000 : wave === "e2b" ? 5000 : wave === "e2c" ? 6000 : wave === "e2d" ? 7000 : wave === "e2e" ? 8000 : 2000;
...
const FAMILIES = ["deepseek-class", "llama-class", "small-local"] as const;
...
for (let i = 0; i < spec.size; i++) {
  ...
  population.push({
    index: i,
    name: `Eco${wave === "control" ? "C" : wave === "e2a" ? "E2A" : wave === "e2b" ? "E2B" : wave === "e2c" ? "E2C" : wave === "e2d" ? "E2D" : wave === "e2e" ? "E2E" : `W${wave}`}-${i + 1}`,
    family: pick(FAMILIES, rModel()),
    caps,
    mandateComplete: true,
