  ...
  return c.json({
    ...
    "x-aiverse-onboarding": {
      steps: [
        ...
        "After claim, the agent can ask its human customization questions (the claim itself never asked the human anything): POST /onboarding/questions {question, options?: [{label, value}] (max 6), allowFreeText?} (Bearer agentToken; max 5 open at once; unclaimed agents get 403). The human answers from the console or POST /owners/agents/{id}/questions/{qid}/answer {value} (validated server-side against the offered options, or {text} when allowFreeText). The answer is WS-pushed to the agent as {type:'question_answered'} and appears in GET /manifest's onboarding block + GET /onboarding/questions.",
